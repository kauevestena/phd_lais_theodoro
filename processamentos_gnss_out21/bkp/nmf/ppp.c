...
/* ,const filopt_t *file_opts) */
{
    ...
    *var=SQR(0.01);
    ...
    fprintf(fout,"ZHD_incl: %f ZWD_incl: %f grad_e: %f grad_n: %f m_w: %f Atraso Inclinado Grad: %f m_w grad: %f Atraso Inclinado: %f ZTD: %f \n", m_h*zhd, m_w*(x[0]-zhd),grad_e,grad_n,m_w-(grad_n*x[1]+grad_e*x[2]),m_h*zhd+m_w*(x[0]-zhd),m_w,m_h*zhd+(m_w-(grad_n*x[1]+grad_e*x[2]))*(x[0]-zhd),x[0]);
    fclose(fout);*/
    ...
    */
    ...
    fprintf(fout,"Time: %i ZHD_incl: %f ZWD_incl: %f grad_e: %f grad_n: %f m_w: %f Atraso Inclinado: %f ZTD: %f \n",time.time,m_h*zhd, m_w*(x[0]-zhd),grad_e,grad_n,m_w,m_h*zhd+m_w*(x[0]-zhd),x[0]);
    fclose(fout);
