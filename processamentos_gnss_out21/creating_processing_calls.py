...
SPECIFY_DURATION = True
START_HOUR = 12
END_HOUR   = 16

def duration_string(start_hour,end_hour,file_datetime,start_min=0,end_min=0):
    # -ts ds ts start day/time (ds=y/m/d ts=h:m:s) [obs start time]
    # -te de te end day/time (de=y/m/d te=h:m:s) [obs end time]
    dt = file_datetime

    ts = f"-ts {dt.year}/{dt.month}/{dt.day} {start_hour}:{start_min}:0 "
    te = f"-te {dt.year}/{dt.month}/{dt.day} {end_hour}:{end_min}:0"
    return ts+te
...
def date_from_rnxdateline(inputrnxline):
    ...
    year,month,day,hour,minute,seconds = as_list[:6]

    return datetime.datetime(year=int(year),month=int(month),day=int(day),hour=int(hour),minute=int(minute),second=int(seconds.split('.')[0]))

def get_rinex_date(inputpath,key='TIME OF FIRST OBS',max_lines=200):
    ...
    for epoch in epochs:
        ...
        for station in stations:
            ...
            for rinexfilename in rinexfilelist:
                if 'o' in rinexfilename.split('.')[1]:
                    if not 'corr2' in rinexfilename:
                        if not '.pos' in rinexfilename:
                            for option in custom_op_modes:
                                ...
                                print('first obs: ',rinex_date,'last obs: ',get_rinex_date(rinex_file_path,'TIME OF LAST OBS'))

                                rinex_gpsweek,rinex_dayofweek = date2gpswd(rinex_date.date())
                                ...
                                if SPECIFY_DURATION:
                                    duration_part = duration_string(START_HOUR,END_HOUR,rinex_date)

                                app_call = f'{rnx2rtkp_path} -k {configuration_path} -o {outpath} {rinex_file_path} {rinex_n_path} {rinex_g_path} {sp3_igs_path} {sp3_igl_path} {clk_path} {blq_filepath} {sat_ant_path} {rcv_ant_path} {dcb_file_path} {duration_part}'
